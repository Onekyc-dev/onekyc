...
export async function getLatestOtpChallenge(email) {
  const { rows } = await getPool().query(
    `select * from email_otp_challenges where email = $1 order by created_at desc limit 1`,
...
export async function createEmailOtp(email) {
  ...
  await getPool().query(
    `update email_otp_challenges set consumed = true where email = $1 and consumed = false`,
    [normalizedEmail]
  );

  await getPool().query(
    `insert into email_otp_challenges (email, code_hash, expires_at) values ($1, $2, $3)`,
...
export async function verifyEmailOtp(email, code) {
  ...
  const { rows } = await getPool().query(
    `select * from email_otp_challenges where email = $1 and consumed = false order by created_at desc limit 1`,
    [normalizedEmail]
  );
  const challenge = rows[0];

  if (!challenge) return { success: false, reason: "missing" };

  if (new Date(challenge.expires_at) < new Date()) {
    await getPool().query(`update email_otp_challenges set consumed = true where id = $1`, [challenge.id]);
  ...
  if (!crypto.timingSafeEqual(Buffer.from(expectedHash, "hex"), Buffer.from(challenge.code_hash, "hex"))) {
    await getPool().query(`update email_otp_challenges set attempts = attempts + 1 where id = $1`, [challenge.id]);
    return { success: false, reason: "invalid" };
  }

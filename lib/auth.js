iimport GoogleProvider from "next-auth/providers/google";
import CredentialsProvider from "next-auth/providers/credentials";
import { getUserByEmail, createUser, verifyEmailOtp } from "./db";
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (!email || !otp) throw new Error("missing");
        ...
        if (!account) {
          account = await createUser({ email, name: null });
        }
  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      if (user?.email) {
        ...
        token.verificationStatus = account?.verificationStatus ?? "none";
        token.flaggedDuplicate = account?.flaggedDuplicate ?? false;
      }
      return token;
    },
    async session({ session, token }) {
      ...
      session.user.flaggedDuplicate = token.flaggedDuplicate;
      return session;
    },
  },
  pages: { signIn: "/" },

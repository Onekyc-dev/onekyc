...
export async function startDiditKyc({ email, returnTo }) {
  if (MOCK_MODE) {
  ...
  const res = await fetch(`${DIDIT_BASE}/v3/session/`, {
    ...
    body: JSON.stringify({
      ...
      vendor_data: email,
      callback: `${process.env.NEXTAUTH_URL}/kyc-processing${returnTo ? `?returnTo=${encodeURIComponent(returnTo)}` : ""}`,
    }),
  });
...
export async function runLivenessCheck({ imageBase64 }) {
 ...
 const res = await fetch(`${DIDIT_BASE}/v3/passive-liveness/`, {
  ...
  if (!res.ok) {
    return { passed: false, reason: "system_error", detail: JSON.stringify(data) };
  }

  const passed = data.status === "Approved";
  return { passed, reason: passed ? null : "face_mismatch", detail: JSON.stringify(data) };
